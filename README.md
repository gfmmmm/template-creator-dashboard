...
Claude Code가 [SETUP.md](SETUP.md) 1부 8단계를 하나씩 이끌어 줍니다. 넉넉히 50분. 대시보드가 끝나면 멈추고, 릴스 대본 기획 도구(2부, 20분)는 준비되셨을 때 "대본 기획 세팅해줘"라고 하면 이어집니다. 컴퓨터 쪽 일은 Claude가 하고, 본인은 가입·클릭·붙여넣기만 합니다. 중간에 쉬어도 돼요 — "이어서 해줘".
...
├── .claude/skills/          2부 — 릴스 대본 기획 도구(릴스대본기획 · 계정세팅)
├── 계정/ · 기획/            2부 — 내 계정 문서(_template 을 복제해 채움) · 완성 대본
├── docs/                    결정 기록 · 기능 레시피
