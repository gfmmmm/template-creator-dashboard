...
검증: `gh secret list`에 키 2개, `gh run list -L 1`에 in_progress 또는 completed. 진행 파일 step=7.
...
## 수업에서 쓸 때 — 세 구간으로 끊어 간다

원데이클래스에서는 한 번에 끝까지 가지 않는다. 강사가 슬라이드에 문장을 띄우면 수강생 전원이 그것을 붙여넣고, Claude는 지정된 단계까지만 가고 멈춘다.

**멈추는 규칙(원칙 10):** "N단계까지 진행해줘"라고 하면 N단계의 검증을 통과한 뒤 멈추고 이렇게 말한다 — "여기까지입니다. 강사님 지시를 기다려 주세요." 다음 구간은 "이어서 진행해줘" 또는 "M단계까지 진행해줘"로 다시 시작한다.

| 구간 | 단계 | 수업 모듈 | 강사가 슬라이드에 띄우는 문장 | 시간 |
|---|---|---|---|---|
| 1 | 0~3 (준비 · 미리보기 · SC 키 · 내 데이터+소스 계정+발굴) | 03 실습1 | `https://github.com/gfmmmm/template-creator-dashboard 에 있는 대시보드를 다운받아서 구축 가이드(SETUP.md)대로 3단계까지 진행해줘. 나는 AI 초보자니까 쉽게 설명해줘.` | 17~25분 |
| 2 | 4 (Gemini 키 · 채널 정체성 · 기둥 분류) | 04 실습1 | `구축 가이드 4단계까지 진행해줘. 나는 AI 초보자니까 쉽게 설명해줘.` | 15분 |
| 3 | 5~7 (GitHub · Vercel · 자동화 · 완료 카드) | 05 | `구축 가이드 5단계부터 끝까지 진행해줘. 나는 AI 초보자니까 쉽게 설명해줘.` | 30~35분 |

- 구간 1의 문장은 아직 폴더를 받기 전에 붙여넣는 것이다. Claude가 ZIP을 풀고 "그 폴더에서 `claude` 를 다시 열어주세요"라고 안내하면, 새로 연 창에 **"구축 가이드 3단계까지 진행해줘"**를 한 번 더 말한다(앞 창의 대화는 새 창으로 넘어가지 않는다). 강사는 이 두 번째 문장도 슬라이드에 같이 띄운다
- 구간 1에서 레퍼런스 발굴까지 끝내는 이유: 04 모듈의 기획 실습이 발굴된 레퍼런스(R-번호)를 재료로 쓴다. 오전에 재료가 없으면 오후 실습이 빈다
